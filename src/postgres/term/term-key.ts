 ...
 * term-key parity suite (`src/postgres/rdfjs-store/term-key-parity.test.ts`)
 * asserts equivalence against the published engine so row keys never diverge
 * between the two packages. Vendored in-repo (the store must not import the
 * engine) per the org convention established by `@worlds/sqlite` and
 * `@worlds/indexeddb`.
 */
